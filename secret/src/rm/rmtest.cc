...
void prepareTuple(const int name_length, const string name, const int age,
		const float height, const int salary, void *buffer, int *tuple_size) {
	int offset = 0;
...
void secA_1(const string tablename, const int name_length, const string name,
		const int age, const float height, const int salary) {
	// Functions tested
	// 1. Create Table ** -- made separate now.
	// 2. Insert Tuple **
	// 3. Read Tuple **
...
void secA_2(const string tablename, const int name_length, const string name,
		const int age, const float height, const int salary) {
	// Functions Tested
	// 1. Insert tuple
	// 2. Delete Tuple **
	...
	cout << "Test Delete Tuple \n";
	// Test Delete Tuple
	rc = rm->deleteTuple(tablename, rid);
	assert(rc == success);

	cout << "Test Read Tuple \n";
	// Test Read Tuple
...
void secA_3(const string tablename, const int name_length, const string name,
		const int age, const float height, const int salary) {
	// Functions Tested
	// 1. Insert Tuple
	// 2. Update Tuple **
...
void secA_4(const string tablename, const int name_length, const string name,
		const int age, const float height, const int salary) {
	// Functions Tested
	// 1. Insert tuple
	// 2. Read Attributes **
...
void secA_5(const string tablename, const int name_length, const string name,
		const int age, const float height, const int salary) {
	// Functions Tested
	// 0. Insert tuple;
	// 1. Read Tuple
...
void secA_31(const string tablename, const int name_length, const string name,
		const int age, const float height, const int salary) {
	// Functions Tested
	// 1. Insert Tuple
	// 2. Update Tuple **
	// 3. Read Tuple
	cout << "****In Test Case 31****" << endl;
	vector<RID> rids;
	vector<int> sizes;
	RID rid;
	int tuple_size = 0;
	int tuple_size_updated = 0;
	void *tuple = malloc(100);
	void *tuple_updated = malloc(500);
	void *data_returned = malloc(500);
	prepareTuple(name_length, name, age, height, salary, tuple, &tuple_size);
	// Test Insert Tuple
	printTuple(tuple, tuple_size);
	RC rc = 0;
	...
	for (int i = 0; i < num_records; i++) {
		...
		assert(rc == success);
		// int size = 0;
		prepareTuple(name_length, name, age, height, salary, tuple,
				&tuple_size);
	...
	cout << "Original RID slot = " << rid.slotNum << endl;
	prepareTuple(288,
			"",
			age, height, 100, tuple_updated, &tuple_size_updated);

	for (int i = 0; i < 2000; i++) {
		// Test Update Tuple
		//cout<<i<<rids[i].pageNum<<rids[i].slotNum<<endl;

		rc = rm->updateTuple(tablename, tuple_updated, rids[i]);
		assert(rc == success);
		rc = rm->readTuple(tablename, rids[i], data_returned);
		assert(rc == success);
		if (memcmp(data_returned, tuple_updated, tuple_size_updated) != 0) {
			cout << "****Update 10 failed****" << endl << endl;
			return;
		} else
			cout << endl << i << "passed\n";
	}
...
void secA_41(const string tablename, const int name_length, const string name,
		const int age, const float height, const int salary) {
	// Functions Tested
	// 1. Insert Tuple
	// 2. Update Tuple **
	// 3. Read Tuple
	cout << "****In Test Case 41****" << endl;
	vector<RID> rids;
	vector<int> sizes;
	RID rid;
	int tuple_size = 0;
	...
	cout << "Original RID slot = " << rid.slotNum << endl;
	prepareTuple(264,
			"",
			age, height, 100, tuple_updated, &tuple_size_updated);
